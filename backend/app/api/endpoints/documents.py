...
from app.services import document_processor, medical_analyzer, firestore_service
from google.cloud import firestore

router = APIRouter()

@router.post("/documents/upload", response_model=models.AnalysisStatus)
async def upload_document(background_tasks: BackgroundTasks, file: Annotated[UploadFile, File()]):
    """
    ...
    try:
        ...
        if file.content_type in ["image/jpeg", "image/png"]:
            extracted_text = await document_processor.extract_text_from_image_with_ocr(gcs_path)
        ...
        background_tasks.add_task(medical_analyzer.analyze_document_in_background, document_id, extracted_text)
