...
class DocumentTypeDetectionAgent:
    ...
    async def detect_document_type(self, extracted_text: str) -> Dict[str, Any]:
        ...
        response = await chain.ainvoke({
            ...
            "extracted_text": extracted_text[:4000]  # Use a slice to avoid overly long prompts
        })
...
class MedicalEntityExtractionAgent:
    def __init__(self):
        self.llm = ChatGoogleGenerativeAI(model=MODEL_NAME, temperature=0.2, response_format={"type": "json_object"})
        self.prompt_template = PromptTemplate.from_template(
    ...
    async def extract_entities(self, extracted_text: str, document_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"Warning: Could not parse JSON from LLM response for entity extraction: {response.content}")
            return []
    ...
    async def retrieve_knowledge(self, entities: List[Dict[str, Any]]) -> List[str]:
        ...
        for entity in entities:
            ...
            if entity_type == "medication":
                knowledge_snippets.append(f"Knowledge about {entity_value}: This is a medication often used for [condition] with common side effects like [side effects].")
            elif entity_type == "lab_test":
                knowledge_snippets.append(f"Knowledge about {entity_value}: This lab test measures [parameter] and has a normal range of [range].")
            elif entity_type == "diagnosis":
                knowledge_snippets.append(f"Knowledge about {entity_value}: This diagnosis is characterized by [symptoms] and is often treated with [treatments].")

        if not knowledge_snippets:
            knowledge_snippets.append("No specific knowledge snippets found for the extracted entities.")

        return knowledge_snippets

class ReasoningAgent:
    def __init__(self):
        self.llm = ChatGoogleGenerativeAI(model=MODEL_NAME, temperature=0.5, response_format={"type": "json_object"})
        self.prompt_template = PromptTemplate.from_template(
    ...
    async def perform_reasoning(self, extracted_text: str, document_type: str, extracted_entities: List[Dict[str, Any]], retrieved_knowledge: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Warning: Could not parse JSON from LLM response for reasoning: {response.content}")
...
class SafetyAssessmentAgent:
    ...
    async def perform_safety_assessment(self, extracted_entities: List[Dict[str, Any]], retrieved_knowledge: List[str]) -> List[Dict[str, Any]]:
        """
        ...
        """
        safety_alerts = []
        medications = [e["entity_value"].lower() for e in extracted_entities if e["entity_type"] == "medication"]
        ...
        for lab in lab_values:
            ...
            try:
                ...
                numeric_part = ''.join(filter(lambda x: x.isdigit() or x == '.', value_str))
                if numeric_part:
                    ...
                    elif "potassium" in value_str.lower() and (value > 5.2 or value < 3.5):
                        safety_alerts.append({
                            "severity": "high",
